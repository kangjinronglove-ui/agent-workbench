...
async function callDeepSeek({ system, input, thinking: t, timeout = 120000 }) {
  const body = JSON.stringify({
    model: "deepseek-v4-pro",
    messages: [{ role: "system", content: system }, { role: "user", content: input }],
    stream: false,
    ...(t ? { thinking: { type: "enabled" } } : { thinking: { type: "disabled" } }),
  });

  return new Promise((resolve, reject) => {
    const req = https.request({
      hostname: "api.deepseek.com", path: "/v1/chat/completions", method: "POST", timeout,
...
async function runPipeline(task, emit) {
  const start = Date.now(); let totalTokens = 0;

  emit({ type: "phase", phase: "architect", status: "running" });
  let design;
  try {
    const r = await callDeepSeek({
      system: "You are a Senior Software Architect. Produce a CONCISE technical design. Include data structures, algorithms, edge cases. Under 200 words. Output ONLY design, NO code. Respond in Chinese if user writes in Chinese.",
      input: `Design a solution for:\n\n${task}`,
      thinking: true, timeout: 180000,
    });
  ...
  try {
    const r = await callDeepSeek({
      system: "You are a Senior Software Engineer. Implement production-ready code. Write COMPLETE code with error handling. Output FULL implementation.",
  ...
  emit({ type: "phase", phase: "reviewer", status: "running", round: 1, max: 1 });
  try {
    const r = await callDeepSeek({
      system: "You are a Code Reviewer. Quickly review code. If correct and handles edge cases: output EXACTLY 'LGTM'. Only flag critical issues (crashes, data loss). Be pragmatic.",
      input: `## Design\n${design}\n\n## Code\n${code}\n\nReview. If approved output LGTM, else list issues.`,
      timeout: 120000,
    });
    totalTokens += r.usage.total_tokens ?? 0;
    if (r.text.trim().toUpperCase().startsWith("LGTM")) {
      emit({ type: "phase", phase: "reviewer", status: "done", approved: true, reasoning: r.reasoning, tokens: r.usage.total_tokens, round: 1 });
      emit({ type: "done", approved: true, code, tokens: totalTokens, time: ((Date.now() - start) / 1000).toFixed(1) });
    } else {
      emit({ type: "phase", phase: "reviewer", status: "done", approved: false, reasoning: r.reasoning, output: r.text, tokens: r.usage.total_tokens, round: 1 });
      emit({ type: "done", approved: false, code, tokens: totalTokens, time: ((Date.now() - start) / 1000).toFixed(1) });
    }
  } catch (e) { emit({ type: "error", message: "Reviewer: " + e.message }); }
}
...
.pipeline{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:32px}
.stage{text-align:center;padding:24px 16px;border-radius:12px;background:#12121f;border:1px solid #1e1e3a;transition:all .4s}
...
.panel-header{padding:16px 22px;background:rgba(255,255,255,.02);border-bottom:1px solid #1e1e3a;font-size:14px;font-weight:700;display:flex;justify-content:space-between;align-items:center}
...
.val.green{color:#22c55e}.val.cyan{color:#06b6d4}.val.yellow{color:#f59e0b}
...
<div class="container">
<div class="header"><h1>AGENT WORKBENCH</h1><p>Architect → Coder → Reviewer &nbsp;↻&nbsp; Auto-Fix</p></div>
<div class="input-group"><input id="task" placeholder="输入编程任务..." value="用Python实现一个支持最大容量和TTL的异步缓存类"><button class="btn-go" id="btn" onclick="run()">▶ Execute</button></div>
<div class="pipeline"><div class="stage" id="s-architect"><span class="icon">🧠</span><div class="name">Architect</div><div class="status">Waiting</div></div><div class="stage" id="s-coder"><span class="icon">✍️</span><div class="name">Coder</div><div class="status">Waiting</div></div><div class="stage" id="s-reviewer"><span class="icon">🔍</span><div class="name">Reviewer</div><div class="status">Waiting</div></div></div>
<div class="output-area" id="output"><div id="panels"></div><div class="report" id="report" style="display:none"></div></div>
</div>
<script>
...
if(e.phase==='architect'){let h='<div class="panel"><div class="panel-header">🧠 Architect — Design<span class="token-badge">'+e.tokens+' tokens</span></div><div class="panel-body">';if(e.reasoning)h+='<div class="reasoning-box">'+esc(e.reasoning.slice(0,800))+'</div>';h+='<div class="code-block">'+esc(e.output)+'</div></div></div>';$('panels').insertAdjacentHTML('beforeend',h);}
if(e.phase==='coder'){$('panels').insertAdjacentHTML('beforeend','<div class="panel"><div class="panel-header">✍️ Coder — Implementation<span class="token-badge">'+e.tokens+' tokens</span></div><div class="panel-body"><div class="code-block">'+esc(e.output)+'</div></div></div>');}
if(e.phase==='reviewer'){if(e.approved){$('panels').insertAdjacentHTML('beforeend','<div class="panel"><div class="panel-body"><div class="lgtm-banner">✅ APPROVED — LGTM</div></div></div>')}else{let h='<div class="panel"><div class="panel-header">🔍 Reviewer — Round '+e.round+'<span class="token-badge">'+e.tokens+' tokens</span></div><div class="panel-body">';if(e.reasoning)h+='<div class="reasoning-box">'+esc(e.reasoning.slice(0,500))+'</div>';const issues=(e.output||'').split('\\n').filter(l=>l.trim());for(const i of issues.slice(0,10))h+='<div class="issue-item">✗ '+esc(i)+'</div>';h+='</div></div>';$('panels').insertAdjacentHTML('beforeend',h);}}
return;
}
if(e.type==='done'){const r=$('report');r.style.display='grid';r.innerHTML='<div class="report-card"><div class="val '+(e.approved?'green':'yellow')+'">'+(e.approved?'✓ APPROVED':'⚠ REVIEW')+'</div><div class="lbl">Verdict</div></div><div class="report-card"><div class="val cyan">'+e.tokens.toLocaleString()+'</div><div class="lbl">Tokens</div></div><div class="report-card"><div class="val yellow">'+e.time+'s</div><div class="lbl">Time</div></div>';}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/run") {
    if (!DEEPSEEK_KEY) {
      res.writeHead(500); return res.end(JSON.stringify({ error: "DEEPSEEK_API_KEY not configured" }));
    }
    let body = "";
...
server.listen(PORT, HOST, () => {
  console.log("Agent Workbench standalone running on http://" + HOST + ":" + PORT);
});
