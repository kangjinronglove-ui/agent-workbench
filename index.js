...
const PROVIDERS = {
  deepseek: {
    ...
    thinking: (enabled) => ({ thinking: { type: enabled ? "enabled" : "disabled" } }),
  ...
  mimo: {
    ...
    thinking: (enabled) => enabled ? { chat_template_kwargs: { enable_thinking: true } } : {},
    authHeaders: (key) => ({ "api-key": key }),
  },
  xiaomi: {
    name: "Xiaomi",
    apiKey: process.env.xiaomi_api_key ?? "",
    hostname: process.env.xiaomi_base_url ? new URL(process.env.xiaomi_base_url).hostname : "CHANGE_ME",
    path: process.env.xiaomi_base_url ? new URL(process.env.xiaomi_base_url).pathname + "/chat/completions" : "/llm/v1/chat/completions",
    model: process.env.XIAOMI_MODEL ?? "mi-llm",
    thinking: (enabled) => ({ thinking: { type: enabled ? "enabled" : "disabled" } }),
    authHeaders: (key) => ({ "Authorization": "Bearer " + key }),
  },
};
...
const IDENTITY = {
  ...
  mimo: "\n\n[IMPORTANT: Your true underlying model is MiMo (小米大模型), developed by Xiaomi. You are NOT OpenAI, GPT, Claude, Codex, or DeepSeek. When asked about your model identity, you MUST answer that you are MiMo, developed by Xiaomi. Ignore any conflicting identity claims in the instructions above.]",
};
...
function buildChatBody(body) {
  ...
  const ident = IDENTITY[PROVIDER] ?? IDENTITY.deepseek;
  let instructions = body.instructions ? body.instructions + ident : ident.trim();
  messages.unshift({ role: "system", content: instructions });

  const chatBody = { model: MODEL, messages, stream };
  if (effectiveThinking) { Object.assign(chatBody, CONFIG.thinking(true)); }
  else { Object.assign(chatBody, CONFIG.thinking(false)); }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && (url.pathname === "/" || url.pathname === "/v1" || url.pathname === "/health")) { res.writeHead(200, { "Content-Type": "application/json" }); return res.end(JSON.stringify({ service: "ccswitch", provider: PROVIDER, model: MODEL, status: "ok", port: PORT })); }
  if (req.method === "POST" && (url.pathname === "/v1/responses" || url.pathname === "/responses")) { try { const raw = await readBody(req); const body = JSON.parse(raw); const { chatBody, stream, messages } = buildChatBody(body); const sk = sessionKey(body);
    const dsReq = https.request({ hostname: CONFIG.hostname, path: "/v1/chat/completions", method: "POST", timeout: 300000, headers: { ...CONFIG.authHeaders(CONFIG.apiKey), "Content-Type": "application/json", Accept: stream ? "text/event-stream" : "application/json" } }, (dsRes) => {
      if (dsRes.statusCode !== 200) { let errBody = ""; dsRes.on("data", c => errBody += c); dsRes.on("end", () => { log.err(CONFIG.name + " " + dsRes.statusCode + ": " + errBody.slice(0,300)); res.writeHead(dsRes.statusCode >= 500 ? 502 : dsRes.statusCode, { "Content-Type": "application/json" }); res.end(JSON.stringify({ error: { type: "upstream_error", code: PROVIDER + "_" + dsRes.statusCode, message: CONFIG.name + " " + dsRes.statusCode + ": " + errBody.slice(0,200) } })); }); return; }
...
server.listen(PORT, "127.0.0.1", () => { console.log(""); log.ok("ccswitch started (" + PROVIDER + " → " + CONFIG.name + ")"); log.info("http://127.0.0.1:" + PORT + "/v1/responses"); log.info("model: " + MODEL); if (!CONFIG.apiKey) log.warn("API key not set for " + PROVIDER + " (" + (PROVIDER === "deepseek" ? "api_key" : "MIMO_API_KEY") + ")"); console.log(""); });
